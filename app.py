...
if st.button("🚀 Kombi-Scan starten"):
    ...
    try:
        ...
        vix_color = "#2ecc71" if vix_price < 20 else "#f1c40f" if vix_price < 30 else "#e74c3c"
        vix_text = f"Marktwetter: <span style='color:{vix_color}; font-weight:bold;'>VIX {vix_price:.2f}</span>"
    except:
        vix_text = "VIX nicht verfügbar"
    ...
    # --- ANZEIGE VIX & ERGEBNISSE ---
    st.markdown(f"### {vix_text}", unsafe_allow_html=True)
    ...
    if price:
        ...
        with p_cols[i % 4]:
            with st.container(border=True):
                ...
                if diff < -15: st.error("⚠️ Call-Gefahr!")
                elif rsi > 60: st.success("🟢 Call-Chance!")
                else: st.info("⏳ Warten")
...
if t_in:
    ...
    if price and dates:
        ...
        try:
            ...
            filtered_df = chain[chain['strike'] <= price * 1.05].sort_values('strike', ascending=False) if mode == "put" else chain[chain['strike'] >= price * 0.95].sort_values('strike', ascending=True)
            for _, opt in filtered_df.head(15).iterrows():
                ...
                st.markdown(f"{risk_emoji} **Strike: {opt['strike']:.1f}** | Bid: <span style='color:#2ecc71; font-weight:bold;'>{bid_val:.2f}$</span> | Delta: {d_abs:.2f} | Puffer: {puffer:.1f}% | Yield: {y_pa:.1f}% p.a.", unsafe_allow_html=True)
        except Exception as e:
            st.error(f"Fehler: {e}")
