...
depot_data = [
    ...
    {"Ticker": "HIMS", "Einstand": 37.0}, {"Ticker": "HOOD", "Einstand": 82.82}, 
    ...
    if price:
        ...
        with p_cols[i % 3]:
            with st.expander(f"{item['Ticker']} ({diff:.1f}%)", expanded=True):
                ...
                elif rsi < 35: st.info("💎 Oversold - Hold")
                if earn: st.caption(f"📅 Earnings: {earn}")

st.write("---") 

# SEKTION 3: EINZEL-CHECK (AMPEL-SYSTEM)
...
if t_in:
    ...
    if price and dates:
        st.info(f"Kurs: **{price:.2f}$** | RSI: **{rsi:.0f}** | Earnings: {earn}")
        ...
        try:
            ...
            for _, opt in filtered_df.head(15).iterrows():
                ...
                # Kompakte Zeile mit HTML für die grüne Farbe der Prämie
                st.markdown(
                    f"{risk_color} **Strike: {opt['strike']:.1f}$** | "
                    f"Bid: <span style='color:#2ecc71
