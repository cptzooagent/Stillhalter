...
def get_stock_basics(symbol):
    try:
        ...
        # Sicherer Earnings-Check ohne Absturzrisiko
        earn_info = ""
        try:
            cal = tk.calendar
            if cal is not None and 'Earnings Date' in cal:
                # Wir holen das Datum sicherheitshalber als String
                dt = cal['Earnings Date'][0]
                earn_info = dt.strftime('%d.%m.')
        except:
            earn_info = ""
...
if st.button("🚀 Markt-Scan starten"):
    ...
    for i, t in enumerate(scan_list):
        ...
        price, dates, earn_info = get_stock_basics(t)
        if price and dates:
            try:
                tk = yf.Ticker(t)
                ...
                if not safe_opts.empty:
                    ...
                    if y_pa >= min_yield_pa:
                        results.append({'ticker': t, 'yield': y_pa, 'strike': best['strike'], 'bid': best['bid'], 'puffer': puffer, 'delta': abs(best['delta_val']), 'days': days, 'earn': earn_info})
            except: continue
...
for i, item in enumerate(depot_data):
    price, _, earn_info = get_stock_basics(item['Ticker'])
    if price:
        diff = (price / item['Einstand'] - 1) * 100
        icon = "🟢" if diff >= 0 else "🟡" if diff > -20 else "🔵"
        with p_cols[i % 4]:
            st.write(f"{icon} **{item['Ticker']}**: {price:.2f}$ ({diff:.1f}%)")
            if earn_info: st.caption(f"Earnings: {earn_info}")
...
if t_in:
    price, dates, earn_info = get_stock_basics(t_in)
    if price and dates:
        if earn_info: st.info(f"📅 Nächste Earnings: {earn_info}")
