...
def get_cached_chain(symbol, date_str, side):
    try:
        ...
        if r.get('s') == 'ok':
            ...
            # Sicherstellen, dass Delta numerisch ist, um Fehler zu vermeiden
...
# 1. TOP 10 SCANNER (FEHLERBEREINIGT)
...
if st.button("🚀 Markt-Scan jetzt starten"):
    ...
    for i, t in enumerate(watchlist):
        ...
        if dates:
            ...
            if df is not None and not df.empty:
                try:
                    # Berechnung mit Schutz gegen ungültige Werte
    ...
    if opps:
        ...
        st.warning("Scan lieferte keine Ergebnisse. API-Daten prüfen.")
